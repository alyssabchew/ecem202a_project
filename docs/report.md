...
* Gesture recognition system:
  * to recognize gestures conducted with the RP2040 using the ML core of the accelerometer/gyroscope chip (LSM6DSOX) and STMICRO's GUI interface to generate and run a         decision tree 
...
From there, we would pass the image and location coordinates into a MATLAB script designed to process the AprilTag and tell us the positioning of the AprilTag. We would use the positions of each AprilTag as waypoints for the system (fixed locations of the smart plugs / devices).  Once per second, we would send the iPhone's current location and orientation to a webhook.  From that webhook, we used IFTTT to compile that data into a Google Sheet that stores the last minute's data with rows demarcating the seconds relative to the global clock.

From these positions, we would then be able to utilize the iPhone's current location and orientation to determine if the iPhone was "pointed at" one of the smart home devices / plugs.  If the iPhone was pointed at a plug, we would listen for a gesture using IFTTT. When the gesture is recognize, the arduino, which is synced with a clock using NTP from tick.ucla.edu, sends a confirmation that the gesture is recognized plus a timestamp for the current second the clock is on adjusted for latency. Thus this allows IFTTT to process the position of the iphone when the gesture is recognized instead of having to constantly keep up with ARKIT updates. This is possible since IFTTT has the past minute's position and orientation data stored on the google sheets mentioned before that houses the last minute's ARKit data.

## Gesture Recognition Model and Training

We utilized Arduino IDE and RP2040 to create a gesture recognition model using STMICROELECTRONIC's UNICO GUI to generate the decision tree.  We initially attempted to define 4 different states (idle, walking, on/off, and all on/all off).  Unfortunately, due to the sensitivity of the RP2040, we found that the on/off gesture was being triggered too frequently.  Because of that, we chose to narrow the number of gestures in the model to 3 with a more robust idle condition. The final product is generated using a idle states trained on a recorded data set of no action, slow walking, and some movement. We also tried injecting gaussian noise into the dataset which improved the decision tree's bounds but did not improve the overall performance upon test. At the end we settled on a window of 52 samples(half a second) and use the features mean and variance for all three axis of the accelerometer. The gyroscope proved difficult to work with when training the model. In the end it is able to recognize side to side movement , up and down flick movement, and idle state. The details for the tree is in "testfin tree." we came to picking different features to train on based on data analysis powered by pandas and python that allowed us to view, cut and extract features from the data.
